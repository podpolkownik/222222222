...
export const Hero = () => {
  ...
  const scrollToStory = () => {
    const storyElement = document.getElementById('story'); // Находим элемент с id="story"
    if (storyElement) {
      window.scrollTo({
        top: storyElement.offsetTop, // Прокручиваем к его верхней части
        ...
        >
          ...
          >
            <motion.div
              ...
              onClick={scrollToStory} // При клике прокручиваем вниз
            >
              <ChevronDown className="w-8 h-8 text-yellow-400 mx-auto cursor-pointer" />
            </motion.div>
          </motion.div>
        </motion.div>
      </div>
